...
foreach my $name (sort keys %pkgs) {
    ...
    if (!$binary) {
        if ($d{special} =~ /in-sources/) { ## requires in-sources install
            ...
            print OUT "$pv-dst: src/$pv ".dep_targets($pkg{dep})."\n\t\@echo \"\" && echo \"*** BUILDING PROJECT: $pv\" && echo \"\" && rm -rf $pv-obj \$\@ && rsync -a src/$pv$srcdir/ $pv-obj/ && cd $pv-obj && ${cfg_chmod}PREFIX=$prefix $cfg_proc ./$cfg_scr ".cfg($pkg{d})." || { c=\$?; if [ -e config.log ]; then echo '== config.log:'; cat config.log; fi; exit \$c; } && PREFIX=$prefix make MAKELEVEL=0 -j$jobs && PREFIX=$prefix $mkinst DESTDIR=$root/build/$pv-dst\n\n";
        } else {
            $cfg_chmod = "chmod $cfg_chmod ".shQuote("../src/$pv$srcdir/$cfg_scr")." && " if ($cfg_chmod ne '');
            print OUT "$pv-dst: src/$pv ".dep_targets($pkg{dep})."\n\t\@echo \"\" && echo \"*** BUILDING PROJECT: $pv\" && echo \"\" && rm -rf $pv-obj \$\@ && mkdir $pv-obj && cd $pv-obj && ${cfg_chmod}PREFIX=$prefix $cfg_proc ../src/$pv$srcdir/$cfg_scr ".cfg($pkg{d})." || { c=\$?; if [ -e config.log ]; then echo '== config.log:'; cat config.log; fi; exit \$c; } && PREFIX=$prefix make MAKELEVEL=0 -j$jobs && PREFIX=$prefix $mkinst DESTDIR=$root/build/$pv-dst\n\n";
        }
